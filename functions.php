...
function isFirstLetter(){           
  if(isset($_GET['filter']) && isset($_GET['letter']) && $_GET['filter']=="a_z"){
    return true;
  }else{
    return false;
  }
...
function isSongsByGenre(){
   if(($_SERVER['REQUEST_URI']=='/genres') && ($_SERVER['argc'] == 0)){
    return true;
  }else{
    return false;
  }
...
function isHits100(){
  if(($_SERVER['REQUEST_URI']=='/top100songs') && ($_SERVER['argc'] == 0)){
    return true;
  }else{
    return false;
  }
...
function isDownloadMp3(){
  //if(($_SERVER['REQUEST_URI']=='/downloadmp3') && ($_SERVER['argc'] == 0)){
  if(($_SERVER['REDIRECT_URL']=='/downloadmp3') && ($_SERVER['argc'] == 0)){
    return true;
  }else{
    return false;
  }
...
function isContacts(){
   if(($_SERVER['REQUEST_URI']=='/contacts') && ($_SERVER['argc'] == 0)){
    return true;
  }else{
    return false;
  }
...
function isCopyright(){
   if(($_SERVER['REQUEST_URI']=='/copyright') && ($_SERVER['argc'] == 0)){
    return true;
  }else{
    return false;
  }
...
function isPlaylists(){
   if(($_SERVER['REQUEST_URI']=='/playlists') && ($_SERVER['argc'] == 0)){
    return true;
  }else{
    return false;
  }
...
function onlyLettersAndSpaces($str){
  return preg_replace ("/[^\p{L}0-9\s]/iu"," ",$str);
}
